...
class LLMPipeline(AbstractLLMPipeline):
    ...
    def _infer_batch(self, sample_batch: Sequence[tuple[str, ...]]) -> list[str]:
        ...
        tokens = self._tokenizer(sample_batch[0], max_length=120, padding=True,
        ...
        for metric in self._metrics:
            ...
            if metric is Metrics.ROUGE:
                ...
                metric_scores['rouge'] = metric.compute(
                    ...
                    predictions=predictions['predictions']).get('rougeL')
            else:
                metric = load(metric.value)
                metric_scores[metric.name] = metric.compute(
                    references=predictions['target'],
                    predictions=predictions['predictions']).get(metric.name)
